                ...
                <form method="POST" action="{{ route('client.store') }}">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <textarea name="details" class="form-control"  id="details" rows="3">{{ $client->details }}</textarea>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="card">
    ...
    <li class="list-group-item">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                @if ($ticket->ticket_type === 'New_lead')
                    <span class="badge badge-success mr-2">{{ $ticket->ticket_type }}</span>
                    Created By: {{ $ticket->created_by }} |  Ticket Number: {{ $ticket->id }} | Ticket Status: {{ $ticket->ticket_status }} | Call Status: {{ $ticket->call_status }}  

                @elseif ($ticket->ticket_type === 'process_ticket')
                    <span class="badge badge-info mr-2">{{ $ticket->ticket_type }}</span>
                    Ticket Status: {{ $ticket->ticket_status }} | Ticket Number: {{ $ticket->id }} | Services Types:
                    @foreach(explode(',', $ticket->services) as $serviceId)
                        {{ \App\Models\Services::find($serviceId)->name }},
                    @endforeach
                    | Clinics Type : {{ $ticket->clinic->name }} | Doctors : {{ $ticket->doctor->name }} | Fees : {{ $ticket->fees }} | Doctor Comment : {{ $ticket->comment }} | Details : {{ $ticket->details }} | Paid Status:{{ $ticket->paid_status }} | Paid Method:{{ $ticket->paid_method }} | Created By: {{ $ticket->created_by }}
                @else
                    <span class="badge badge-danger mr-2">{{ $ticket->call_status }}</span>
                    Ticket Number: {{ $ticket->id }} | Created By: {{ $ticket->created_by }} 
                @endif
            </div>
            <div class="d-flex">
